...
class AllParkins extends Component {
    ...
    componentDidMount() {
        ...
        database.child("AllParking").on("child_added", (snap) => {
            let obj = snap.val()
            obj.id = snap.key
    ...
    deleteParking(data, indexNum) {
        ...
        database.child(`AllParking/${data.id}`).remove()
        database.child(`Parkings/${data.parkinID}/bookingArr/${data.index}/`).set(slots)
    }

    render() {
        this.props.BookingLis.allBooking.map((newVal, index) => {
            let slots = {
                active: false,
                index: newVal.index,
            }
